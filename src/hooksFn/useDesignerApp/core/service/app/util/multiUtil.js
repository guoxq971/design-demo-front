export class MultiUtil {
  /**
   * 复杂多角度处理
   * @param {array} list 图片列表
   * @return {array} resultList 处理后的图片列表
   * */
  static complexMultiDispose(list) {
    // 打上组的标签 composeGroup
   ...
   * */
  static findMultiDesignImg(multiDesignImgList, composeId, multiId) {
    let url = '';
    // 复杂多角度查找
    if (composeId) {
  ...
  // 解析多角度
  static parseMulti(appearance, renderMultiList = []) {
    // 简单多角度
