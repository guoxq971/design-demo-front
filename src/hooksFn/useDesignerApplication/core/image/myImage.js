...
export const { exclusives, _isExclusive } = useExclusive();

export const useGlobalMyImage = createGlobalState(() => {
  const list = ref([]);
  const total = ref(0);
  ...
  async function getList() {
    ...
    // 专属共享类判断
    const { isExclusive, __params } = _isExclusive(_params);
  ...
  async function _normalGetList(_params) {
    const config = {
      url: apis.image_list_by_my(_params),
      method: METHOD.GET,
      // params: _params,
    };
  ...
  async function _exclusiveGetList(_params) {
    const config = {
      url: apis.image_list_by_exclusive(_params),
      method: METHOD.GET,
      // params: _params,
    };
...
function useAccountListService() {
  ...
  async function getList() {
    const config = {
      url: apis.account_list(),
      method: METHOD.POST,
      // params: _params,
    };
    let arr = await AppUtil.fetch(config, { about, loading });
    arr = arr.filter((e) => e.id !== '').map((item) => ({ label: item.name, value: item.id, right: item.userName }));
    arr.unshift({ label: exclusives.label, value: exclusives.value, right: '' });
