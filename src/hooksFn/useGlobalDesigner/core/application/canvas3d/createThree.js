...
const THREE_CAMERA_FOCAL_LENGTH = 50;

/**
 * @typedef {import('d').threeTemplate}
 */
export class CreateThree {
   ...
   * @param {number} [opt.lightIntensity=0.28] 灯光强度
  ...
  create(opt) {
    opt = Object.assign(
      {
        ...
        hdr: '/img.png', //hdr路径
        id: '', //容器id
        lightIntensity: 0.28, //灯光强度
        createType: '', //创建类型
      },
      opt,
    );
    // console.log('灯光', opt.lightIntensity);
    // 容器
    ...
    // 相机的位置
    let cameraPosition = { x: -1, y: 2, z: -4.5 };
    // 默认值放大3倍
    cameraPosition.x *= 3;
    cameraPosition.y *= 3;
    cameraPosition.z *= 3;
    this.camera.position.set(cameraPosition.x, cameraPosition.y, cameraPosition.z);
    this.camera.setFocalLength(THREE_CAMERA_FOCAL_LENGTH);
   ...
   */
  destroy() {
    if (this.destroyMouse) this.destroyMouse();
